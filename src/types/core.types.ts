...
export interface VitalSigns {
  ...
  readonly respiratoryRate?: number; // per minute
  readonly oxygenSaturation?: number; // percentage
  readonly bloodGlucose?: number; // mg/dL
  readonly timestamp: Date;
}
...
export interface TriageDecision {
  ...
  readonly estimatedWaitTime: number; // minutes
...
// Additional supporting types (condensed for brevity)
...
export interface OutcomeImpact {
  readonly patientHealth: number; // -1 to 1
  readonly cost: number;
  readonly time: number; // minutes
  readonly satisfaction: number; // 0 to 1
}
